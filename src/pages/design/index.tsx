...
import { DesignView, PageDesign } from "@/types/design";
...
import ModalFilter from "@/components/modal/modal-filter-design";
...
const DropdownMore: NextPage<CellContext<DesignView, unknown> & PropsDropdownMore> = ({
  row,
  toggleModalDelete,
}) => {
    ...
    <div className="relative inline-block py-2 text-right" ref={refMore}>
      ...
      <div className={`z-50 absolute right-0 mt-2 w-56 rounded-md overflow-hidden origin-top-right shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none duration-300 ease-in-out ${!moreBar && 'scale-0 shadow-none ring-0'}`}>
        <div className="" role="none">
          <Link href={{ pathname: '/design/[id]', query: { id: row.original.id } }}>
...
const Index: NextPage<Props> = () => {
  ...
  const [design, setDesign] = useState<DesignView[]>([]);
  const [showModalFilter, setShowModalFilter] = useState<boolean>(false);
  const [showModalDelete, setShowModalDelete] = useState<boolean>(false);
  const [deleteId, setDeleteId] = useState<string>('');
  const [deleteVerify, setDeleteVerify] = useState<string>('');

  const [filter, setFilter] = useState<PageDesign>({
    name: '',
    description: '',
    startDt: '',
    endDt: '',
  ...
  const [pageRequest, setPageRequest] = useState<PageDesign>({
    limit: 10,
    page: 1,
    preloads: "Company,Order",
  });

  const column: ColumnDef<DesignView>[] = [
    {
      id: 'name',
      accessorKey: 'name',
      header: () => {
  ...
  const { isLoading, data, refetch } = useQuery({
    queryKey: ['design', pageRequest],
    queryFn: ({ queryKey }) => Api.get('/design', queryKey[1] as object),
  });
  ...
  return (
    <>
      <Head>
        <title>{process.env.APP_NAME + ' - Design'}</title>
      </Head>
      <ModalFilter
