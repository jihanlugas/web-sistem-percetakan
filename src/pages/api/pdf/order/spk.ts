...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    res.setHeader('Content-Disposition', `inline; filename=${body.name.replaceAll(" ", "-").toLowerCase()}.pdf`);
    ...
    doc.fontSize(16).text("Surat Perintah Kerja", {
      align: "center",
      underline: true,
    })
    ...
    doc.text("Customer Service", currX, currY);
    doc.text(": " + body.createName, currX + 150, currY);

    // currX = doc.x
    currY = doc.y

    doc.text("Tanggal", currX, currY);
    doc.text(": " + displayDateTime(new Date()), currX + 150, currY);

    doc.moveDown(2)

    doc.text("Hallo", currX)
    ...
    tableHeaders.forEach((header, index) => {
      doc.fontSize(12).text(header, 50 + index * columnWidth, doc.y, { width: columnWidth, align: 'center' });
    });
    ...
    tableData.forEach(row => {
      row.forEach((cell, index) => {
        doc.text(cell, 50 + index * columnWidth, doc.y, { width: columnWidth, align: 'center' });
      });
      doc.moveDown();
    });
